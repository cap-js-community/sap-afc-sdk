...
public class SchedulingProcessingHandlerSimpleMockCompletedTest {
    ...
    void processJob() throws Exception {
        ...
        List<JobResult> jobResults = persistenceService.run(
                Select.from(JOB_RESULT).where(jr -> jr.job_ID().eq(ID))
        ...
        List<String> messageResultIDs = jobResults.stream()
                .filter(r -> Objects.equals(r.getTypeCode(), ResultTypeCode.MESSAGE))
                .map(JobResult::getId)
                .toList();

        List<JobResultMessage> jobResultMessages = persistenceService.run(
                Select.from(JOB_RESULT_MESSAGE).where(jr -> jr.result_ID().eq(messageResultIDs.get(0)))
        ...
        jobResultMessages = persistenceService.run(
                Select.from(JOB_RESULT_MESSAGE).where(jr -> jr.result_ID().eq(messageResultIDs.get(1)))
        ).listOf(JobResultMessage.class);
        assertEquals(1, jobResultMessages.size());
        ...
        String textResultID = jobResults.stream()
                .filter(r -> "text/plain".equals(r.getMimeType()))
                .map(JobResult::getId)
                .findFirst()
                .orElseThrow();

        JobResult jobResult = persistenceService.run(Select.from(Scheduling_.JOB_RESULT).byId(textResultID)).single(cds.gen.scheduling.JobResult.class);
        assertEquals("Job completed successfully", new String(jobResult.getData().readAllBytes()));

        String pdfResultID = jobResults.stream()
                .filter(r -> "application/pdf".equals(r.getMimeType()))
                .map(JobResult::getId)
                .findFirst()
                .orElseThrow();

        jobResult = persistenceService.run(Select.from(Scheduling_.JOB_RESULT).byId(pdfResultID)).single(cds.gen.scheduling.JobResult.class);
