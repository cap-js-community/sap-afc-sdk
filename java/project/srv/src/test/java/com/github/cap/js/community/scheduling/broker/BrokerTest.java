...
public class BrokerTest {
    ...
    void getCatalog() throws Exception {
        MvcResult result = mockMvc.perform(get("/broker/v2/catalog").accept("application/json"))
                .andExpect(status().isOk())
                .andReturn();
        mockMvc.perform(asyncDispatch(result))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.services[0].id")
                        .value("c48d9d0b-4320-439e-8f4e-2ab0294bd971"))
                .andExpect(jsonPath("$.services[0].plans[0].id")
                        .value("a7609272-ce22-41e0-a605-1ad3318ef2d5"));
    }

    @Test
    void getCatalogNoAuth() throws Exception {
        mockMvc.perform(get("/broker/v2/catalog"))
