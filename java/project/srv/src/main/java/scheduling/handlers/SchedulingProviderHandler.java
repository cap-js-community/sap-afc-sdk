...
public class SchedulingProviderHandler implements EventHandler {
    ...
    public void beforeCreateJob(CdsCreateEventContext context, List<Job> jobs) {
        ...
        Select<JobDefinition_> query = Select.from(JOB_DEFINITION).columns(CQL.star(), CQL.to("parameters").expand()).where(jd -> jd.name().eq(definitionName));
        ...
        for (JobParameterDefinition jobParameterDefinition : jobDefinition.getParameters()) {
            ...
            if (!ParameterTypeCode.MAPPING.equals(jobParameterDefinition.getTypeCode())) {
                if (ParameterTypeCode.READ_ONLY_VALUE.equals(jobParameterDefinition.getTypeCode()) && parameter.containsKey("value") && parameter.get("value") != null && !parameter.get("value").equals(jobParameterDefinition.getValue())) {
                ...
                switch (jobParameterDefinition.getDataTypeCode()) {
                    ...
                    case DataTypeCode.NUMBER:
                        try {
                            ...
                            throw JobSchedulingError.jobParameterValueInvalidType(parameter.get("value"), parameter.getName(), jobParameterDefinition.getDataTypeCode());
                        }
                        parameter.put("value", parameter.get("value").toString());
                        break;
                    ...
                    case DataTypeCode._BOOLEAN:
                        if (!(("true".equals(parameter.get("value").toString())) || ("false".equals(parameter.get("value").toString())))) {
                            throw JobSchedulingError.jobParameterValueInvalidType(parameter.get("value"), parameter.getName(), jobParameterDefinition.getDataTypeCode());
                        }
                        parameter.put("value", parameter.get("value").toString());
                        break;
        ...
        if (jobDefinition.getSupportsTestRun()) {
            ...
            Optional<JobParameterDefinition> testRunParameterDefinition = jobDefinition.getParameters().stream().filter(parameter -> MappingTypeCode.TEST_RUN.equals(parameter.getMappingTypeCode())).findFirst();
            if (testRunParameterDefinition.isPresent()) {
                Optional<JobParameter> testRunParameter = job.getParameters().stream().filter(parameter -> testRunParameterDefinition.get().getName().equals(parameter.getDefinitionName())).findFirst();
    ...
    public void createJob(CdsCreateEventContext context, List<Job> jobs) {
        ...
        Select<cds.gen.schedulingproviderservice.Job_> read = Select.from(cds.gen.schedulingproviderservice.SchedulingProviderService_.JOB).byId(job.getId());
