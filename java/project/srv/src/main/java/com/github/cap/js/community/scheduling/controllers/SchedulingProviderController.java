...
public class SchedulingProviderController {
    ...
    public List<JobDefinition> jobDefinitions(@RequestParam(name = "skip", required = false) Integer skip,
                                              @RequestParam(name = "top", required = false) Integer top,
                                              @RequestParam(name = "name", required = false) String name,
                                              @RequestParam(name = "search", required = false) String search) {
        Select<JobDefinition_> query = Select.from(JOB_DEFINITION)
                .orderBy(jd -> jd.name().asc());
        ...
        if (search != null && !search.isEmpty()) {
            ...
            query = query.where(jd -> jd.name().toLower().plain("like", "'" + pattern + "'")
                    .or(jd.description().toLower().plain("like", "'" + pattern + "'")
    ...
    public JobDefinition jobDefinition(@PathVariable(name = "name", required = true) String name, HttpServletResponse response) {
    ...
    public List<JobParameterDefinition> jobDefinitionParameters(@PathVariable(name = "name", required = true) String name,
        ...
        Select<JobParameterDefinition_> query = Select.from(JOB_PARAMETER_DEFINITION).where(jpd -> jpd.jobName().eq(name))
    ...
    @Tag(name = "Job")
    @GetMapping("/Job")
    public List<Job> jobs(@RequestParam(name = "skip", required = false) Integer skip,
                          @RequestParam(name = "top", required = false) Integer top,
                          @RequestParam(name = "name", required = false) String name,
                          @RequestParam(name = "referenceID", required = false) String referenceID) {
        Select<Job_> query = Select.from(JOB)
                .orderBy(j -> j.name().asc());
        query = applyLimit(query, top, skip);
    ...
    public List<JobParameter> jobParameters(@PathVariable(name = "ID", required = true) String ID,
                                            @RequestParam(name = "skip", required = false) Integer skip,
                                            @RequestParam(name = "top", required = false) Integer top,
                                            HttpServletResponse response) {
        ...
        Select<JobParameter_> query = Select.from(JOB_PARAMETER).where(jp -> jp.jobID().eq(ID))
                .orderBy(jd -> jd.jobID().asc(), jd -> jd.name().asc());
        query = applyLimit(query, top, skip);
        List<JobParameter> parameters = providerService.run(query).listOf(JobParameter.class);

        Select<JobParameterDefinition_> jpdQuery = Select.from(JOB_PARAMETER_DEFINITION).where(jpd -> jpd.jobName().eq(job.get().getName()));
        Stream<JobParameterDefinition> jpds = providerService.run(jpdQuery).streamOf(JobParameterDefinition.class);

        Map<String, JobParameterDefinition> jpsMap = jpds
                .collect(Collectors.toMap(JobParameterDefinition::getName, Function.identity()));
    ...
    public List<JobParameter> jobResults(@PathVariable(name = "ID", required = true) String ID,
                                         @RequestParam(name = "skip", required = false) Integer skip,
                                         @RequestParam(name = "top", required = false) Integer top,
                                         HttpServletResponse response) {
        Select<Job_> existsQuery = Select.from(JOB).byId(ID);
        ...
        Select<JobResult_> query = Select.from(JOB_RESULT).where(jobResult -> jobResult.jobID().eq(ID))
    ...
    public JobResult jobResult(@PathVariable(name = "ID", required = true) String ID,
                               HttpServletResponse response) {
    ...
    public List<JobResultMessage> jobResultMessages(@PathVariable(name = "ID", required = true) String ID,
                                                    @RequestParam(name = "skip", required = false) Integer skip,
                                                    @RequestParam(name = "top", required = false) Integer top,
                                                    HttpServletResponse response) {
        ...
        Select<JobResultMessage_> query = Select.from(JOB_RESULT_MESSAGE).where(jrm -> jrm.resultID().eq(ID))
    ...
    public StreamingResponseBody jobResultData(@PathVariable(name = "ID", required = true) String ID,
                                               HttpServletResponse response) {
        Select<cds.gen.scheduling.JobResult_> query = Select.from(cds.gen.scheduling.Scheduling_.JOB_RESULT).byId(ID);
        Optional<cds.gen.scheduling.JobResult> _jobResult = persistenceService.run(query).first(cds.gen.scheduling.JobResult.class);
        if (_jobResult.isEmpty()) {
            response.setStatus(HttpStatus.NOT_FOUND.value());
            return null;
        }
        cds.gen.scheduling.JobResult jobResult = _jobResult.get();
        String dispositionType = cdsModel.getEntity(cds.gen.scheduling.JobResult_.CDS_NAME).getElement("data").getAnnotationValue("@Core.ContentDisposition.Type", "attachment");
    ...
    @Tag(name = "Job")
    @PostMapping("/Job")
    public ResponseEntity<?> createJob(@RequestBody String body, HttpServletRequest request, HttpServletResponse response) {
            ...
            try {
                ...
                if (exception instanceof CdsException) {
                    ...
                    HttpStatus status = Optional.ofNullable(
                            HttpStatus.resolve(error.getErrorStatus().getHttpStatus())
                    ).orElse(HttpStatus.INTERNAL_SERVER_ERROR);
                    return ResponseEntity
                            .status(status)
                            .contentType(MediaType.APPLICATION_JSON)
                            .body(Map.of(
                                    "code", error.getPlainMessage(),
                                    "message", error.getLocalizedMessage(request.getLocale())
                            ));
                } else {
                    return ResponseEntity
